...
func testGroupCompactE2e(t *testing.T, mergeFunc storage.VerticalChunkSeriesMergeFunc) {
	objtesting.ForeachStore(t, func(t *testing.T, bkt objstore.Bucket) {
		...
		planner := NewPlanner(logger, []int64{1000, 3000}, noCompactMarkerFilter, nil, TombstoneCompactionRatio)
		...
		bComp, err := NewBucketCompactor(logger, sy, grouper, planner, comp, dir, bkt, nil, 2, true)
		testutil.Ok(t, err)
