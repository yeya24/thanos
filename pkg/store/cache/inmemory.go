...
func NewInMemoryIndexCache(logger log.Logger, reg *promauto.Factory, conf []byte) (*InMemoryIndexCache, error) {
...
// size approximately does not exceed maxBytes.
func NewInMemoryIndexCacheWithConfig(logger log.Logger, reg *promauto.Factory, config InMemoryIndexCacheConfig) (*InMemoryIndexCache, error) {
	...
	c.evicted = reg.NewCounterVec(prometheus.CounterOpts{
	...
	c.added = reg.NewCounterVec(prometheus.CounterOpts{
	...
	c.overflow = reg.NewCounterVec(prometheus.CounterOpts{
	...
	c.current = reg.NewGaugeVec(prometheus.GaugeOpts{
	...
	c.currentSize = reg.NewGaugeVec(prometheus.GaugeOpts{
	...
	c.totalCurrentSize = reg.NewGaugeVec(prometheus.GaugeOpts{
