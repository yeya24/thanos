...
func newBucketStoreMetrics(reg promauto.Factory) *bucketStoreMetrics {
	...
	m.seriesDataFetched = reg.NewSummaryVec(prometheus.SummaryOpts{
	...
	m.seriesDataSizeTouched = reg.NewSummaryVec(prometheus.SummaryOpts{
	...
	m.seriesDataSizeFetched = reg.NewSummaryVec(prometheus.SummaryOpts{
	...
	m.seriesBlocksQueried = reg.NewSummary(prometheus.SummaryOpts{
		Name: "thanos_bucket_store_series_blocks_queried",
		Help: "Number of blocks in a bucket store that were touched to satisfy a query.",
	})
	m.seriesGetAllDuration = reg.NewHistogram(prometheus.HistogramOpts{
	...
	m.seriesMergeDuration = reg.NewHistogram(prometheus.HistogramOpts{
		Name:    "thanos_bucket_store_series_merge_duration_seconds",
		Help:    "Time it takes to merge sub-results from all queried blocks into a single result.",
		Buckets: []float64{0.001, 0.01, 0.1, 0.3, 0.6, 1, 3, 6, 9, 20, 30, 60, 90, 120},
	})
	m.resultSeriesCount = reg.NewSummary(prometheus.SummaryOpts{
		Name: "thanos_bucket_store_series_result_series",
		Help: "Number of series observed in the final result of a query.",
	})

	m.chunkSizeBytes = reg.NewHistogram(prometheus.HistogramOpts{
...
func NewBucketStore(
	...
	metrics := newBucketStoreMetrics(promauto.With(reg))
	s := &BucketStore{
		logger:               logger,
		bkt:                  bucket,
		fetcher:              fetcher,
