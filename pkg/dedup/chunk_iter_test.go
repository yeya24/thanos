...
func TestDedupChunkSeriesMergerDownsampledChunks(t *testing.T) {
	m := NewDedupChunkSeriesMerger()
	//toAggrChunks := func(t *testing.T, cm []chunks.Meta) (res []*downsample.AggrChunk) {
	//	for i := range cm {
	//		achk, ok := cm[i].Chunk.(*downsample.AggrChunk)
	...
	//samples3 := downsample.SamplesFromTSDBSamples(createSamplesWithStep(0, 25, 5*60*1000))
